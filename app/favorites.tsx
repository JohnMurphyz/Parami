...
export default function FavoritesScreen() {
  ...
  // Filter favorites based on selected paramis
  const filteredFavorites = selectedParamiIds.length === 0
    ? favorites
    : favorites.filter(fav => selectedParamiIds.includes(fav.paramiId));
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.content}>
        ...
        <ScreenHeader
          ...
          subtitle={`${filteredFavorites.length} item${filteredFavorites.length !== 1 ? 's' : ''} saved`}
          onBack={() => router.back()}
          renderFilter={() => (
        ...
        {filteredFavorites.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateTitle}>
              {selectedParamiIds.length > 0 ? 'No Matching Items' : 'Treasures to Return To'}
            </Text>
            <Text style={styles.emptyStateText}>
              {selectedParamiIds.length > 0
                ? 'Try adjusting your filter to see more saved items.'
